...
def get_breaks(pages: List[str]) -> List[Dict[str, time]]:
    ...
    for page in pages:
        ...
        indefinite_break = re.search(
            r"\(\s*Przerwa\s+w\s+posiedzeniu\s+o\s+godz\.\s+(\d+)\s+min\s+(\d+)\s*\)",
            page,
        )
        ...
        if indefinite_break:
            start_hour = int(indefinite_break.group(1))
            start_minute = int(indefinite_break.group(2))
            breaks.append({"start": time(start_hour, start_minute)})
